...
Additional **runtime adapters** are siblings of `kaula-runtime` against the
same contracts — `kaula-runtime-langgraph` (`kaula.runtime_langgraph`) adds
LangGraph/LangChain support with native pause/resume via graph interrupts.
...
- **LangGraph runtime** ([`kaula-runtime-langgraph`](packages/kaula-runtime-langgraph/README.md))
  — heal tools in LangGraph/LangChain, with native pause/resume via graph
  interrupts. (CrewAI adapter: [`kaula-runtime`](packages/kaula-runtime/README.md).)
